#!/usr/bin/env python3
# Disclaimer: This script is for educational purposes only.  Do not use against any network that you don't own or have authorization to carry out a test attack on.

# We will use the subprocess module to run commands on Kali Linux.
import subprocess
import re
# We want to open the CSV files generated by airmon-ng, 
# and we'll use the built-in csv module.
import csv
# We will have to import os because we want to check for sudo
import os
# using time.sleep()
import time
# We want to move .csv files in the folder if we found any. 
# We'll need to use shutil to do that.
import shutil
# We'll create a timestamp for .csv filename
...
print(r"""
___  ___  ___  ___    ___  _ _    ___  _ _  ___  ___  ___  ___   ___  _ _ 
|  _]| . || . \| __]  | . ]| | |  |  _]| | || . ]| __]| . \/  _] | __]| \ |
| [__| | || | || _]   | . \\   /  | [__\   /| . \| _] |   /| [_/\| _] |   |
...
# If the user doesn't run the program with super user privileges, program should end immediately.
if not 'SUDO_UID' in os.environ.keys():
    print("You must be root to run this program.Please use sudo su.")
    exit()
...
# Regex to find wireless interfaces. hopefully they will all be wlan0 or higher.
...
# If no WiFi Adapter connected.
if len(check_wifi_result) == 0:
    print("Please connect a WiFi adapter and run this program again.")
    exit()

# Menu to select WiFi interface from
print("The following WiFi interfaces were found:")
...
# For easy reference we call the selected interface hakerflee
